...
static int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true) {
		if (pinfo != NULL && pinfo->chargerid_vol_disable != true) {
			chargerid_volt = mt_vadc_read(
				10, Channel_13); //get the charger id volt
...
static void oplus_get_chargeric_temp_volt(struct charger_data *pdata)
{
	...
	chargeric_temp_volt =
		mt_vadc_read(1, Channel_14); //get the typec tem adc volt
...
int oplus_chg_set_qc_config(void)
{
	...
	if (!chip->calling_on && !chip->camera_on &&
		...
		if (oplus_chg_check_vbus_change(7600,
						true)) //icl checkpoint is 7600
			ret = 0;
	} else {
		if (chip->charger_volt > 7500 &&
			...
			if (oplus_chg_check_vbus_change(
				    5800, false)) //icl checkpoint is 5800
				ret = 0;
		}
	}
