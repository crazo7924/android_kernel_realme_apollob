...
int mt_get_chargerid_volt(struct oplus_chg_chip *chip)
{
	...
	if (is_support_chargerid_check() == true) {
		chargerid_volt =
			mt_vadc_read(10, Channel_14); //get the charger id volt
...
void mt_set_usb_shell_ctrl_val(struct oplus_chg_chip *chip, int value)
{
	...
	if (value) {
		gpio_direction_output(chip->usb_shell_ctrl_gpio.usb_shell_gpio,
				      1);
		pinctrl_select_state(
			chip->usb_shell_ctrl_gpio.pinctrl,
	...
	chg_err("set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chip->usb_shell_ctrl_gpio.usb_shell_gpio));
}
...
int oplus_usb_shell_ctrl_switch_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg_err("---2-----");
	chip->usb_shell_ctrl_gpio.usb_shell_ctrl_active =
		pinctrl_lookup_state(chip->usb_shell_ctrl_gpio.pinctrl,
				     "usb_shell_ctrl_output_high");
	if (IS_ERR_OR_NULL(chip->usb_shell_ctrl_gpio.usb_shell_ctrl_active)) {
		chg_err("get chargerid_switch_active fail\n");
		return -EINVAL;
	}

	chip->usb_shell_ctrl_gpio.usb_shell_ctrl_sleep = pinctrl_lookup_state(
		chip->usb_shell_ctrl_gpio.pinctrl, "usb_shell_ctrl_output_low");
