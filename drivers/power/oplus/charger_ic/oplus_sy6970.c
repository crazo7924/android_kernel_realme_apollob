...
int sy6970_adc_start(struct sy6970 *bq, bool oneshot)
{
	...
	if (oneshot)
		ret = sy6970_update_bits(
			bq, SY6970_REG_02, SY6970_CONV_START_MASK,
			SY6970_CONV_START << SY6970_CONV_START_SHIFT);
	else
		ret = sy6970_update_bits(
			bq, SY6970_REG_02, SY6970_CONV_RATE_MASK,
...
int sy6970_set_chargecurrent(struct sy6970 *bq, int curr)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_04, SY6970_ICHG_MASK,
...
int sy6970_set_term_current(struct sy6970 *bq, int curr)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_05, SY6970_ITERM_MASK,
...
int sy6970_set_prechg_current(struct sy6970 *bq, int curr)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_05, SY6970_IPRECHG_MASK,
...
int sy6970_set_chargevolt(struct sy6970 *bq, int volt)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_06, SY6970_VREG_MASK,
...
int sy6970_set_input_volt_limit(struct sy6970 *bq, int volt)
{
	...
	sy6970_update_bits(bq, SY6970_REG_0D, SY6970_FORCE_VINDPM_MASK,
			   SY6970_FORCE_VINDPM_ENABLE
				   << SY6970_FORCE_VINDPM_SHIFT);

	return sy6970_update_bits(bq, SY6970_REG_0D, SY6970_VINDPM_MASK,
...
int sy6970_set_watchdog_timer(struct sy6970 *bq, u8 timeout)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_07, SY6970_WDT_MASK, val);
}
...
int sy6970_reset_watchdog_timer(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_03, SY6970_WDT_RESET_MASK,
				  val);
}
...
int sy6970_force_dpdm(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_FORCE_DPDM_MASK,
				 val);
...
int sy6970_reset_chip(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_14, SY6970_RESET_MASK, val);
	return ret;
}
...
int sy6970_enter_hiz_mode(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_00, SY6970_ENHIZ_MASK, val);
}
...
int sy6970_disable_enlim(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_00, SY6970_ENILIM_MASK, val);
}
...
static int sy6970_inform_charger_type(struct sy6970 *bq)
{
	...
	if (!bq->mtk_master_chg) {
		bq->mtk_master_chg =
			power_supply_get_by_name("mtk-master-charger");
		if (!bq->mtk_master_chg) {
	...
	ret = power_supply_set_property(bq->mtk_master_chg,
	...
	if (!ret && prev_chg_type != bq->chg_type && bq->chg_det_enable) {
		...
		if (bq->chg_type == CHARGER_UNKNOWN) {
			...
			cancel_delayed_work_sync(
				&bq->sy6970_current_setting_work);
		}
...
int irq_gpio_init_sy6970(struct sy6970 *bq)
{
	...
	bq->irq_gpio_pinctrl.irq_active = pinctrl_lookup_state(
		bq->irq_gpio_pinctrl.pinctrl, "irq_default");

	if (IS_ERR_OR_NULL(bq->irq_gpio_pinctrl.irq_active)) {
		chg_err("get irq_default fail\n");
		return -EINVAL;
	}
	bq->irq_gpio_pinctrl.irq_sleep = pinctrl_lookup_state(
		bq->irq_gpio_pinctrl.pinctrl, "irq_set_low");
...
static int bq2589x_get_charging_status(struct sy6970 *bq,
				       enum bq2589x_charging_status *chg_stat)
{
	int ret = 0;
	u8 val;
...
static int sy6970_enter_ship_mode(struct sy6970 *bq, bool en)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_09, SY6970_BATFET_DIS_MASK,
				 val);
	return ret;
}
...
int oplus_sy6970_set_input_current_limit(int current_ma)
{
	...
	if (current_ma && diff.tv_sec < 10) {
		...
		schedule_delayed_work(&g_bq->sy6970_aicr_setting_work,
				      msecs_to_jiffies(ms));
	} else {
		cancel_delayed_work(&g_bq->sy6970_aicr_setting_work);
...
static int bq2589x_charger_get_online(struct sy6970 *bq, bool *val)
{
	bool pwr_rdy = false;
				...
				const struct i2c_device_id *id)
{
	...
	bq->chg_consumer = charger_manager_get_by_name(&client->dev, "sy6970");
	...
	INIT_DELAYED_WORK(&bq->sy6970_current_setting_work,
