...
void do_chrdet_int_task(void)
{
	...
	if (oplus_gauge_check_chip_is_null() ||
	    oplus_vooc_check_chip_is_null() || oplus_chg_check_chip_is_null()) {
		battery_xlog_printk(
			BAT_LOG_CRTI,
	...
	if (oplus_vooc_get_fastchg_started() == true &&
		...
		battery_xlog_printk(
			BAT_LOG_CRTI,
			"[do_chrdet_int_task] opchg_get_prop_fast_chg_started = true!\n");
		return;
	}
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[do_chrdet_int_task] usb_present:%d\n",
				    usb_present);
	} else {
		battery_xlog_printk(
			BAT_LOG_CRTI,
...
static long adc_cali_unlocked_ioctl(struct file *file, unsigned int cmd,
				    unsigned long arg)
{
	int *user_data_addr;
	int *naram_data_addr;
	...
	static int at_i_sense = 0, at_current_offset = 0,
		   at_charger_off_vol = 5000 * 5;
	...
	case ADC_CHANNEL_READ:
		...
		;
		battery_xlog_printk(BAT_LOG_CRTI,
				    "**** unlocked_ioctl : ADC_CHANNEL_READ\n");
	...
	case BAT_STATUS_READ:
		...
		battery_xlog_printk(BAT_LOG_CRTI,
				    "**** unlocked_ioctl : BAT_STATUS_READ\n");
		break;
	...
	case Set_Charger_Current: /* For Factory Mode*/
		...
		battery_xlog_printk(
			BAT_LOG_CRTI,
			"**** unlocked_ioctl : set_Charger_Current\n");
		break;
	//add for meta tool-------------------------------
...
#ifdef CONFIG_COMPAT
static long adc_cali_compat_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	long ret = 0;
	...
	case COMPAT_TEST_ADC_CALI_PRINT:
		ret = file->f_op->unlocked_ioctl(
			file, COMPAT_TEST_ADC_CALI_PRINT, (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_SET_ADC_CALI_Slop:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_SET_ADC_CALI_Slop,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_SET_ADC_CALI_Offset:
		ret = file->f_op->unlocked_ioctl(
			file, COMPAT_SET_ADC_CALI_Offset, (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_SET_ADC_CALI_Cal:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_SET_ADC_CALI_Cal,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_ADC_CHANNEL_READ:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_ADC_CHANNEL_READ,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_BAT_STATUS_READ:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_BAT_STATUS_READ,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Set_Charger_Current: /* For Factory Mode*/
		ret = file->f_op->unlocked_ioctl(
			file, COMPAT_Set_Charger_Current, (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Get_META_BAT_VOL:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_Get_META_BAT_VOL,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Get_META_BAT_SOC:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_Get_META_BAT_SOC,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Get_FakeOff_Param: /* For Factory Mode*/
		ret = file->f_op->unlocked_ioctl(file, COMPAT_Get_FakeOff_Param,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Get_Notify_Param: /* For Fakeoff Mode*/
		ret = file->f_op->unlocked_ioctl(file, COMPAT_Get_Notify_Param,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_Turn_Off_Charging: /* For Turnoffcharging Mode*/
		ret = file->f_op->unlocked_ioctl(file, COMPAT_Turn_Off_Charging,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_K_AT_CHG_CHGR_IN:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_K_AT_CHG_CHGR_IN,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_K_AT_CHG_CHGR_OFF:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_K_AT_CHG_CHGR_OFF,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_K_AT_CHG_ON:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_K_AT_CHG_ON,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_K_AT_CHG_OFF:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_K_AT_CHG_OFF,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_K_AT_CHG_INFO:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_K_AT_CHG_INFO,
						 (unsigned long)arg64);
		if (ret < 0) {
	...
	case COMPAT_SET_SPI_CS_LOW:
		ret = file->f_op->unlocked_ioctl(file, COMPAT_SET_SPI_CS_LOW,
						 (unsigned long)arg64);
		if (ret < 0) {
