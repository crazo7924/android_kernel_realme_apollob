...
int oplus_bq2589x_set_ichg(int cur)
{
	...
	} else if (g_bq->chg_cur != 0 && chip->icharging < -300 &&
		   chip->ui_soc > 98) {
	...
	if ((g_oplus_chip != NULL) &&
		...
		if (g_bq->oplus_chg_type == POWER_SUPPLY_TYPE_USB) {
			g_oplus_chip->sub_chg_ops->charging_disable();
		} else if (g_oplus_chip->sub_chg_ops->charging_enable() ==
			   false) {
			g_oplus_chip->sub_chg_ops->charging_enable();
		}
	}
...
int oplus_bq2589x_set_qc_config(void)
{
	...
	if (!g_bq->pdqc_setup_5v && chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		...
		if ((chip->batt_volt <
		     chip->limits.vbatt_pdqc_to_5v_thr - 100) &&
