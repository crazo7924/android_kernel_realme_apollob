...
static int oplus_wrx_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->wrx_en_active =
		pinctrl_lookup_state(chg->wrx_en_pinctrl, "wrx_en_active");
	...
	chg->wrx_en_sleep =
		pinctrl_lookup_state(chg->wrx_en_pinctrl, "wrx_en_sleep");
	...
	chg->wrx_en_default =
		pinctrl_lookup_state(chg->wrx_en_pinctrl, "wrx_en_default");
...
static int oplus_wrx_otg_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->wrx_otg_active =
		pinctrl_lookup_state(chg->wrx_otg_pinctrl, "wrx_otg_active");
	...
	chg->wrx_otg_sleep =
		pinctrl_lookup_state(chg->wrx_otg_pinctrl, "wrx_otg_sleep");
...
void oplus_set_wrx_otg_value(int value)
{
	...
	if (value == 1) {
		...
		pinctrl_select_state(chg->wrx_otg_pinctrl, chg->wrx_otg_active);
	} else {
		gpio_direction_output(chg->wrx_otg_gpio, 0);
		pinctrl_select_state(chg->wrx_otg_pinctrl, chg->wrx_otg_sleep);
	}
	...
	chg_err("<~WPC~> set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chg->wrx_otg_gpio));
}
...
void oplus_set_wrx_en_value(int value)
{
	...
	if (value == 1) {
		...
		pinctrl_select_state(chg->wrx_en_pinctrl, chg->wrx_en_active);
	} else {
		///gpio_direction_output(chg->wrx_en_gpio, 0);
		pinctrl_select_state(chg->wrx_en_pinctrl, chg->wrx_en_sleep);
	}

	chg_err("<~WPC~> set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chg->wrx_en_gpio));
}
...
get_type_done:
	if (chip && chip->wireless_support &&
	    oplus_wireless_charge_start() == true)
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:11111 tongfeng test qcom,wrx_en-gpio!\n",
		       __func__);
		chg->wrx_en_gpio =
			of_get_named_gpio(node, "qcom,wrx_en-gpio", 0);
		if (chg->wrx_en_gpio <= 0) {
			chg_err("Couldn't read qcom,wrx_en-gpio rc=%d, \n", rc);
		} else {
			if (1) { //oplus_mps_otg_en_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->wrx_en_gpio,
						  "qcom,wrx_en-gpio");
				if (rc) {
	...
	if (chip) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:11111 tongfeng test qcom,wrx_otg-gpio!\n",
		       __func__);
		chg->wrx_otg_gpio =
			of_get_named_gpio(node, "qcom,wrx_otg-gpio", 0);
		if (chg->wrx_otg_gpio <= 0) {
			chg_err("Couldn't read qcom,wrx_otg-gpio rc=%d, \n",
				rc);
		} else {
			if (1) { //oplus_mps_otg_en_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->wrx_otg_gpio,
						  "qcom,wrx_otg-gpio");
				if (rc) {
...
bool oplus_chg_is_usb_present(void)
{
	...
	if ((chip->pmic_spmi.smb5_chip->chg.typec_mode ==
	    ...
	    chip->vbatt_num == 2 && oplus_wireless_charge_start() == false) {
